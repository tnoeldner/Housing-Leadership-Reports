-- Supabase tables for storing engagement report data and analyses
-- These tables enable historical analysis, trending, and reporting for Residence Life Events
...
-- Using a simpler constraint that works better with Supabase upsert
...
-- Create unique constraint for analyses (one analysis per week per user per type)
...
-- RLS (Row Level Security) policies for engagement_report_data
...
-- Policy to allow authenticated users to read all engagement data (for analysis)
CREATE POLICY "Allow authenticated users to read engagement data" 
ON engagement_report_data FOR SELECT 
TO authenticated 
USING (true);

-- Policy to allow authenticated users to insert their own engagement data
...
-- RLS policies for saved_engagement_analyses
...
-- Policy to allow authenticated users to read all saved analyses
CREATE POLICY "Allow authenticated users to read engagement analyses" 
ON saved_engagement_analyses FOR SELECT 
TO authenticated 
USING (true);

-- Policy to allow users to insert/update their own analyses
...
COMMENT ON COLUMN engagement_report_data.event_date IS 'The actual date when the event is scheduled/occurred';
COMMENT ON COLUMN engagement_report_data.report_date IS 'The date when the form was submitted/reported';
COMMENT ON COLUMN engagement_report_data.event_status IS 'Status of the event: planned, completed, cancelled';
COMMENT ON COLUMN engagement_report_data.attendance_count IS 'Number of participants/attendees';
COMMENT ON COLUMN engagement_report_data.programming_theme IS 'Theme or category of programming (educational, social, wellness, etc.)';
COMMENT ON COLUMN saved_engagement_analyses.upcoming_events IS 'Formatted list of upcoming events extracted from the analysis period';