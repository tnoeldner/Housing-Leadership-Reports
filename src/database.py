        ...
        try:
            ...
            # Check for RLS error and try fallback
                ...
                try:
                    ...
                    response = admin_client.table("saved_staff_recognition").insert(save_data).execute()
                    if response.data:
                        return {
                            "success": True, 
                            "message": f"Staff recognition saved for week ending {week_ending_date} (via admin override)",
                            "saved_id": response.data[0]['id']
                        }
                except Exception as admin_e:
                    admin_error_msg = str(admin_e)
                    # Check if admin override also hit duplicate key
                    if "duplicate key" in admin_error_msg or "23505" in admin_error_msg:
                        return {
                            "success": True,
                            "message": f"Recognition for week ending {week_ending_date} already exists (no duplicate created)"
                        }
